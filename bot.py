...
def quality_status(theatrical, digital, bluray):
    if bluray:
...
async def trending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, m in enumerate(results, 1):
        ...
        keyboard.append([InlineKeyboardButton(f"📋 {title}", callback_data=f"movie_{m['id']}")])
...
async def search_movie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, m in enumerate(results, 1):
        ...
        rating = m.get("vote_average", 0)
        text += f"{i}. *{title}* ({year}) ⭐{rating:.1f}\n"
        keyboard.append([InlineKeyboardButton(f"📋 {title} ({year})", callback_data=f"movie_{m['id']}")])
    await update.message.reply_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def search_drama(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, m in enumerate(results, 1):
        ...
        rating = m.get("vote_average", 0)
        text += f"{i}. *{title}* ({year}) ⭐{rating:.1f}\n"
        keyboard.append([InlineKeyboardButton(f"📋 {title} ({year})", callback_data=f"tv_{m['id']}")])
    await update.message.reply_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))

async def browse_drama(update, context, code, label):
    await update.message.reply_text(f"🔍 Mencari drama {label}...")
    data = tmdb_get("/discover/tv", {"with_origin_country": code, "sort_by": "popularity.desc"})
    ...
    for i, m in enumerate(results, 1):
        ...
        rating = m.get("vote_average", 0)
        text += f"{i}. *{title}* ({year}) ⭐{rating:.1f}\n"
        keyboard.append([InlineKeyboardButton(f"📋 {title}", callback_data=f"tv_{m['id']}")])
    await update.message.reply_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))

async def korea(update, context):
    await browse_drama(update, context, "KR", "Korea 🇰🇷")

async def china(update, context):
    await browse_drama(update, context, "CN", "China 🇨🇳")

async def japan(update, context):
    await browse_drama(update, context, "JP", "Jepang 🇯🇵")
...
async def text_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    movies = [r for r in movie_data.get("results", []) if r.get("original_language") != "id"][:3]
    tvs = [r for r in tv_data.get("results", []) if r.get("original_language") != "id"][:3]
...
async def notif_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📲 *Pilih notifikasi:*", parse_mode="Markdown", reply_markup=keyboard)

async def send_coming_soon(bot):
    ...
    for i, m in enumerate(results, 1):
        ...
        text += f"{i}. *{title}*\n📅 {date} | ⭐{rating:.1f}\n\n"
        keyboard.append([InlineKeyboardButton(f"📋 {title}", callback_data=f"movie_{m['id']}")])
    await bot.send_message(chat_id=OWNER_CHAT_ID, text=text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))

async def send_now_playing(bot):
...
async def send_trending_now(bot):
    ...
    for i, m in enumerate(results, 1):
        ...
        keyboard.append([InlineKeyboardButton(f"{media} {title}", callback_data=f"{ctype}_{m['id']}")])
    await bot.send_message(chat_id=OWNER_CHAT_ID, text=text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_movie_detail(query, movie_id):
    ...
    theatrical, digital, bluray = get_release_info(movie_id)
    hours = runtime // 60
    mins = runtime % 60
    duration = f"{hours}j {mins}m" if hours else f"{mins}m"
    text = (
        f"🎬 *{title}* ({year})\n"
        f"⭐ *{rating:.1f}/10* | ⏱ {duration}\n"
        f"🎭 {genres}\n\n"
        f"📖 {overview}\n\n"
        f"👥 *Cast:* {cast_str}\n\n"
        f"📅 *Bioskop:* {theatrical or '-'}\n"
        f"📺 *Digital:* {digital or '-'}\n"
        f"💿 *Bluray:* {bluray or '-'}\n\n"
        f"🎯 {quality_status(theatrical, digital, bluray)}"
    )
    poster = item.get("poster_path")
    justwatch_url = f"{JUSTWATCH_BASE}{requests.utils.quote(title)}"
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📺 Where to Watch", url=justwatch_url)],
        [InlineKeyboardButton("🎬 Trailer", url=f"https://www.youtube.com/results?search_query={requests.utils.quote(title+' '+year+' trailer')}")]
    ])
    if poster:
        await query.message.reply_photo(photo=f"{TMDB_IMG}{poster}", caption=text, parse_mode="Markdown", reply_markup=keyboard)
    else:
...
async def show_tv_detail(query, tv_id):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📺 Where to Watch", url=justwatch_url)],
        [InlineKeyboardButton("🎬 Trailer", url=f"https://www.youtube.com/results?search_query={requests.utils.quote(title+' trailer')}")]
    ])
    if poster:
        await query.message.reply_photo(photo=f"{TMDB_IMG}{poster}", caption=text, parse_mode="Markdown", reply_markup=keyboard)
    else:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("movie_"):
        await show_movie_detail(query, data.split("_")[1])
    elif data.startswith("tv_"):
        await show_tv_detail(query, data.split("_")[1])
    elif data == "notif_coming":
        await query.message.reply_text("⏳ Mengambil data...")
        await send_coming_soon(context.bot)
    elif data == "notif_playing":
        await query.message.reply_text("⏳ Mengambil data...")
        await send_now_playing(context.bot)
    elif data == "notif_trending":
        await query.message.reply_text("⏳ Mengambil data...")
        await send_trending_now(context.bot)
